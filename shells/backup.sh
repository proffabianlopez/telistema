...
BACKUP_DIR_DB="../database_dump/"
REMOTE_DIR_DB="/home/telistema/database_dump"
REMOTE_USER="root"
...
send_email() {
    ...
    if [ $? -eq 0 ]; then
        echo -e "\e[32m$(date +%Y-%m-%d\ %H:%M:%S) Correo enviado exitosamente a $EMAIL!\e[0m"
    else
        echo -e "\e[31mFallo al enviar el correo a $EMAIL\e[0m"
    fi
}

# Función para generar el dump de MySQL
backup_mysql() {
    echo -e "\e[34m$(date +%Y-%m-%d\ %H:%M:%S) Paso 1: Iniciando MYSQLDUMP...\e[0m"
    ...
    echo -e "\e[32m$(date +%Y-%m-%d\ %H:%M:%S) Paso 1: Completado!\e[0m"
}

# Función para comprimir el archivo
compress_backup() {
    echo -e "\e[34m$(date +%Y-%m-%d\ %H:%M:%S) Paso 2: Comprimendo archivo...\e[0m"
    ...
    echo -e "\e[32m$(date +%Y-%m-%d\ %H:%M:%S) Paso 2: Completado!\e[0m"
}
...
move_local_backup() {
    ...
    echo -e "\e[32m$(date +%Y-%m-%d\ %H:%M:%S) Paso 3: Backup movido localmente!\e[0m"
}

# Función para copiar el archivo al VPS
copy_to_vps() {
    echo -e "\e[34m$(date +%Y-%m-%d\ %H:%M:%S) Paso 4: Copiando al VPS...\e[0m"
    ...
    echo -e "\e[32m$(date +%Y-%m-%d\ %H:%M:%S) Paso 4: Backup copiado al VPS!\e[0m"
}

# Función para eliminar backups antiguos en el VPS
clean_remote_backups() {
    echo -e "\e[34m$(date +%Y-%m-%d\ %H:%M:%S) Paso 5: Eliminando backups antiguos en el VPS...\e[0m"
    ...
    if [ -z "$FILES" ]; then
        echo -e "\e[33m$(date +%Y-%m-%d\ %H:%M:%S) No se encontraron backups antiguos en el VPS para eliminar.\e[0m"
    else
        ...
        echo -e "\e[32m$(date +%Y-%m-%d\ %H:%M:%S) Paso 5: Completado! Backups antiguos eliminados en el VPS!\e[0m"
    fi
}

# Función para eliminar backups locales antiguos
clean_local_backups() {
    echo -e "\e[34m$(date +%Y-%m-%d\ %H:%M:%S) Paso 6: Eliminando backups locales antiguos...\e[0m"
    ...
    if ls $BACKUP_DIR_DB/$BACKUP_CLIENT._BKP_.$DATE_PREV* 1> /dev/null 2>&1; then
        ...
        echo -e "\e[32m$(date +%Y-%m-%d\ %H:%M:%S) Paso 6: Backups locales antiguos eliminados!\e[0m"
    else
        echo -e "\e[33m$(date +%Y-%m-%d\ %H:%M:%S) No se encontraron backups locales antiguos para eliminar.\e[0m"
    fi
}
...
echo "---------------------------------------------------------------------------"
echo -e "\e[34m$(date +%Y-%m-%d\ %H:%M:%S) Inicio de backup ...\e[0m"
echo "---------------------------------------------------------------------------"
...
echo "---------------------------------------------------------------------------"
echo -e "\e[32m$(date +%Y-%m-%d\ %H:%M:%S) ...Backup completado!!\e[0m"
echo "---------------------------------------------------------------------------"
