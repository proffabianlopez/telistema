...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_GET['action'] === 'edit_order_technic') {
        if (empty($_POST['id_state_order'])) {
            ...
            // Validar si hay imágenes o no
            if (isset($_FILES['name_image']) && count($_FILES['name_image']['name']) > 0 && !empty($_FILES['name_image']['name'][0])) {
            ...
            // Continuar independientemente de si hay imagen o no
            if ($stmt->execute()) {
