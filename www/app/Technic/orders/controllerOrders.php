...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (empty($_POST['id_state_order'])) {
        ...
        if (isset($_FILES['name_image']) && !empty($_FILES['name_image']['name'][0])) {
            ...
            $contador = 1; // Inicializa el contador para las imágenes
            ...
            foreach ($_FILES['name_image']['tmp_name'] as $key => $archivo_temporal) {
                if ($_FILES['name_image']['error'][$key] === UPLOAD_ERR_OK) {
                    ...
                    $tipo_archivo = strtolower(pathinfo($_FILES['name_image']['name'][$key], PATHINFO_EXTENSION));
                    $fecha_actual = date('dmY'); // Fecha en formato DDMMAAAA

                    // Generar el nombre de la imagen con el contador
                    $nuevo_nombre = "Ord:{$id_order}_{$fecha_actual}_{$contador}.{$tipo_archivo}";

                    $ruta_imagen = $directorio_destino . $nuevo_nombre;

                    // Validar el tipo de archivo
                    if (!in_array($tipo_archivo, $tipos_permitidos)) {
                        $response['message'] = 'El tipo de archivo no es permitido: ' . $_FILES['name_image']['name'][$key];
                        echo json_encode($response);
                        exit;
                    }
                    ...
                    if (file_exists($ruta_imagen)) {
                        $response['message'] = 'El archivo ' . $nuevo_nombre . ' ya existe.';
                        echo json_encode($response);
                        exit;
                    }
                    ...
                    if (move_uploaded_file($archivo_temporal, $ruta_imagen)) {
                        ...
                        $response['message'] = 'Error al subir el archivo: ' . $_FILES['name_image']['name'][$key];
                        echo json_encode($response);
                        exit;
                    }
                } else {
                ...
                // Incrementar el contador después de procesar la imagen
                $contador++; 
            }
